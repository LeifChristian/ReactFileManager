...
app.get("/getFiles", (req, res) => {
  ...
  if (API_SECRET !== req.query.string) {
...
app.post("/writeFile", (req, res, next) => {
  ...
  if (API_SECRET !== req.body.API_SECRET) {
    console.log("invalid auth");
    res.send("2");
    return;
  }
...
app.post("/createFile", (req, res, next) => {
  ...
  if (req.body.newCreatedFileName == "") {
...
app.post("/renameDirectory", (req, res, next) => {
  ...
  if (req.body.oldDirName.length > 0 && req.body.newDirName.length > 0) {
    if (req.body.directory == "") {
      ...
      res.send(
        `${req.body.directory}/${req.body.oldDirName} renamed to ${req.body.directory}/${req.body.newDirName}`
      );
    }
...
const go = async (fileToConvertToText) => {
  ...
  let file = fs.readFileSync(
    path.resolve(__dirname, `${fileToConvertToText}`),
    "utf8"
  );
